class MedianFinder {
    ...
    public void addNum(final int num) {
        ...
        // rebalance the size of the two halves as needed
        rebalance();
    }
    ...
    private void rebalance() {
        // no rebalancing needed if sizes are within 1 of each other
        if(Math.abs(high.size() - low.size()) <= 1) {
            return;
        }

        // move one element from the larger (by quantity) half into the other half to equalize sizes
        if(high.size() > low.size()) {
            low.add(high.poll());
        } else {
            high.add(low.poll());
